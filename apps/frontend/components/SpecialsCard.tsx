...
export function SpecialsCard({
  ...
  return (
    <article className="min-w-0 w-full max-w-full rounded-xl shadow overflow-hidden bg-white border border-neutral-light">
      ...
      <div className="p-4 min-w-0">
        <div className="flex items-baseline justify-between gap-2 mb-1 min-w-0">
          <h2 className="font-heading font-bold text-lg leading-tight min-w-0 truncate text-neutral">
            {item.title}
          </h2>
          {costDisplay != null && (
        ...
        {item.description && (
          <div className="font-body text-sm text-text-paragraph">
            <p
              ref={descriptionRef}
        ...
        <div className="my-3 border-t border-neutral-light" />
        <div className="flex flex-wrap items-center gap-2 min-w-0">
          <button
            type="button"
            onClick={() => onSave?.(item)}
            className="p-1.5 rounded text-neutral-500 hover:text-primary"
            aria-label="Save to list"
          >
            <Bookmark className="w-5 h-5" />
          </button>
          <button type="button" className="p-1.5 rounded text-neutral-500 hover:text-primary" aria-label="Share">
            <Share2 className="w-5 h-5" />
          </button>
          <Link
            href={`/venues/${item.venueId}`}
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full font-body text-sm font-semibold bg-neutral-900 text-white hover:bg-neutral-800"
          >
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full font-body text-sm font-semibold bg-white text-neutral-900 hover:bg-neutral-100 border border-neutral-300"
          >
