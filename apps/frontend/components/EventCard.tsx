  ...
  return (
    <article className="min-w-0 w-full max-w-full rounded-xl shadow overflow-visible bg-white dark:bg-neutral-900 border border-neutral-light dark:border-neutral-700">
      ...
      <div className="px-3 sm:px-4 pb-4 min-w-0">
        ...
        {showAttendance ? (
          <div className="flex w-full min-w-0 items-center gap-3 sm:gap-4 pl-2 sm:pl-3 pb-0.5 sm:pb-0">
            ...
            <div className="relative min-w-0 shrink" ref={attendanceMenuRef}>
                <button
                  type="button"
                  ...
                  className={`inline-flex min-w-0 max-w-full items-center gap-1 px-2 py-2 sm:gap-1.5 sm:px-3 sm:py-2 rounded-full font-body text-xs sm:text-sm font-semibold border transition-colors disabled:opacity-50 ${
                    goingActive
                  ...
                  aria-haspopup="menu"
                  title={!goingActive && !interestedActive ? 'Set attendance' : undefined}
                >
                  {goingActive ? (
                    <>
                      <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
                      <span className="min-w-0 truncate">Going</span>
                    </>
                  ) : interestedActive ? (
                    <>
                      <Star className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
                      <span className="min-w-0 truncate">Interested</span>
                    </>
                  ) : (
                    <>
                      <Calendar className="w-4 h-4 sm:w-5 sm:h-5 shrink-0 opacity-70" />
                      <span className="min-w-0 truncate">RSVP</span>
                    </>
                  )}
                  <ChevronDown className={`w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 opacity-70 transition-transform ${attendanceMenuOpen ? 'rotate-180' : ''}`} />
                </button>
              {attendanceMenuOpen && (
                <div
                  role="menu"
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 px-2 py-2 sm:gap-1.5 sm:px-3 sm:py-2 rounded-full font-body text-xs sm:text-sm font-semibold bg-neutral-900 dark:bg-neutral-700 text-white hover:bg-neutral-800 dark:hover:bg-neutral-600 whitespace-nowrap"
