...
## 6. Types (TypeScript) — `lib/types.ts`
...
Use `lib/types.ts` as the canonical type definitions for Venue, Experience, MenuSection, MenuItem when syncing with another codebase.

---

## 7. Handle validation and generation

- **Validation:** Implement the same rules as in `lib/handleUtils.ts` (length 3–40, charset a–z 0–9 `-`, no leading/trailing/consecutive hyphens) and uniqueness against `venues` where `handle == value`.
- **Generation from name:** Lowercase, strip accents, replace non-allowed chars with nothing, spaces with single hyphen, collapse hyphens, trim leading/trailing, slice to 40 chars. See `generateHandleFromName` in `lib/handleUtils.ts`.
...
- [ ] **Types:** Copy or re-generate from `lib/types.ts` (Venue, MenuItem, MenuSection, Experience, etc.) so both codebases stay aligned.

---

**Version:** 1.0  
**Matches:** Current admin behaviour and `lib/types.ts` as of this changelog’s date.
